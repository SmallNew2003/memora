# l1-search-retrieval Specification

## Purpose
为 memora L1 临时记忆层定义三套检索接口的契约：`search`（基于 FTS5 + BM25 跨 observation / summary 排序）、`recent_observations`（按 `(created_at DESC, id DESC)` 拉取近期观察，支持按 session 过滤）、`recent_sessions`（按 `(created_at DESC, id DESC)` 拉取近期会话）。本 spec 固化响应字段集合、字段顺序与错误码（`SESSION_NOT_FOUND` / `INVALID_INPUT` / `STORAGE_ERROR`），使后续 capability profiles / procedural skill compiler / L2 桥接变更在叠加能力时不必破坏既有契约。本 spec 不引入 sqlite-vec / embedding / 语义检索；error message 不得包含绝对路径、未脱敏的 observation / summary 内容或本地文件系统信息。
## Requirements
